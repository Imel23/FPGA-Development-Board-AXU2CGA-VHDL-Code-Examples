...
#
# Script generated by Vivado on Sat Jun 07 13:22:26 CEST 2025
...
#
# Usage        : fifo_ip.sh
#                fifo_ip.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                fifo_ip.sh [-reset_run]
#                fifo_ip.sh [-reset_log]
#                fifo_ip.sh [-help]
#
...
#                script (type 'fifo_ip.sh -help' for more information).
#
...
# script info
echo -e "fifo_ip.sh - Script generated by export_simulation (Vivado v2024.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./fifo_ip.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  xmelab $xmelab_opts $design_libs_elab xil_defaultlib.fifo_ip xil_defaultlib.glbl
}

# RUN_STEP: <simulate>
simulate()
{
  xmsim $xmsim_opts xil_defaultlib.fifo_ip -input simulate.do
}
...
reset_run()
{
  files_to_remove=(xmvlog.log xmvhdl.log xmsc.log compile.log elaborate.log simulate.log diag_report.log xsc_report.log fifo_ip_sc.so .tmp_log xcelium_lib waves.shm c.obj)
...
Usage: fifo_ip.sh [-step]\n\
Usage: fifo_ip.sh [-lib_map_path]\n\
Usage: fifo_ip.sh [-reset_run]\n\
Usage: fifo_ip.sh [-reset_log]\n\
Usage: fifo_ip.sh [-keep_index]\n\
